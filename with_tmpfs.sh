...
[ "$((source_size_in_kb*2))" -gt "$for_tmpfs" ] && [ -n "$(grep '/var/tmp/portage' /etc/fstab )" ] || sed -i '/\/var\/tmp\/portage/d' /etc/fstab && umount /var/tmp/portage

# If doubled source size is less then size of tmpfs and tmpfs was not previously mounted, then we mount tmpfs

[ "$((source_size_in_kb*2))" -lt "$for_tmpfs" ] && [ -z "$(grep '/var/tmp/portage' /etc/fstab )" ] && printf "tmpfs\t/var/tmp/portage\ttmpfs\tsize=${for_tmpfs}K,uid=portage,gid=portage,mode=775,noatime\t0 0" >> /etc/fstab && mount /var/tmp/portage
